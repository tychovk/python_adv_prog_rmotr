...
def create_ships(board_state = None):
    ...
    '''
    global comp_sub, comp_aircraft, comp_patrol1, comp_patrol2, player_sub, player_aircraft, player_patrol1, player_patrol2, all_ships, ships_left, comp_ships, player_ships
...
def answer_confirmation(message, answers):
...
def shooting(act_player, board_state, currently_damaged_ship = False):
    '''
    ...
    if act_player == "computer":
        ...
        sides_tried = [0,0]
        if not focus_fire:
            shoot_x, shoot_y = random.randrange(10), random.randrange(10)
        else:
            while True:    
                print ("The enemy is focus firing!")
                if len(focus_fire) == 1:
                    range_diff = -1 ** random.randrange(0,2)
                    x_or_y = random.randrange(0,2)
                    shoot_x_and_y = list(focus_fire[0])
                    shoot_x_and_y[x_or_y] += range_diff
                    shoot_x, shoot_y = shoot_x_and_y[0], shoot_x_and_y[1]
                    if shoot_x < 0 or shoot_y < 0:
                        continue
                        
                elif len(focus_fire) > 1:
                    if focus_fire[0][0] == focus_fire[1][0]:
                        x_or_y = 1
                    elif focus_fire[0][1] == focus_fire[1][1]:
                        x_or_y = 0
                    reference_bomb_point = -1 * random.randrange(0,2)
                    new_bombing_cell = list(focus_fire[reference_bomb_point])
                    if reference_bomb_point == 0:
                        second_ref_point = 1
                    elif reference_bomb_point == -1:
                        second_ref_point = -2
            
                    ff_1 = focus_fire[reference_bomb_point][x_or_y]
                    ff_2 = focus_fire[second_ref_point][x_or_y]
                    
                    new_bombing_cell[x_or_y] = 2 *  ff_1 - ff_2
                    shoot_x = new_bombing_cell[1]
                    shoot_y = new_bombing_cell[0]
                
                focus_cell = board_state[1][shoot_y][shoot_x][0]
                if shoot_x in range(10) and shoot_y in range(10) and focus_cell != "x":
                    break
                else:
                    sides_tried[x_or_y] += 1
                    if 0 in sides_tried:
                        continue
                    else:
                        shoot_x, shoot_y = random.randrange(10), random.randrange(10)
        ...
        shoot_board = 0
        hit_message = "We hit a ship! We don't know what ship it is, but good job sir!"
        shoot_x = shoot_request("Give us an order where to shoot! What's the X coordinate?") - 1
        shoot_y = shoot_request("What's the Y coordinate?") - 1
    
    if board_state[shoot_board][shoot_y][shoot_x][0] == ' ':
        if act_player == "human":
            confirmation = answer_confirmation("You gave us the coordinates {x}, {y}. Ready!? (y/n)".format(x = shoot_x + 1, y = shoot_y + 1), ('y','n'))
            if confirmation == 'n':
    ...
    board_state[shoot_board][shoot_y][shoot_x][0], board_state[shoot_board][shoot_y][shoot_x][2] = "x", "x"
    hit_ship_check = hit_ship(shoot_y, shoot_x, board_state, act_player)
    
    if hit_ship_check == False:

        if act_player == "computer":
            pause = input("The enemy hit none of our ships this time. Shall we see what we can do?")
        if act_player == "human":
            pause = input("We did not hit any of their ships this time. Let's see what our enemy is up to.")
        return board_state
    else:
        board_state[shoot_board][shoot_y][shoot_x][1] = 'X'
        print (hit_message.format(hit_ship_check[1]))
        if act_player == "computer":
            focus_fire.append([shoot_x, shoot_y])
            max_length_vessel_left = max([len(x) for x in ships_left[1]])
            if len(focus_fire) == max_length_vessel_left:
                focus_fire = []
...
def hit_ship(guess_y, guess_x, board_state, active_player):
    searchlist = enemy_ship_list(active_player)
    for ship in searchlist:
        if [guess_y, guess_x] in ship:
...
def new_game():
    ...
    New game state
    '''
    global comp_sub, comp_aircraft, comp_patrol1, comp_patrol2, player_sub, player_aircraft, player_patrol1, player_patrol2, all_ships, ships_left, focus_fire
